...
valgrind  -s --leak-check=full --show-leak-kinds=all  --verbose  --show-reachable=yes  --track-origins=yes --log-file="$SAVED/sys_tests/run_logs/_valgrind-6383.log"  src/redis-server  --bind 0.0.0.0 --port 6383 --dir ./data --dbfilename 6383.rdb --logfile "$SAVED/sys_tests/run_logs/_valgrind-redis-6383.log" --maxmemory 4G --protected-mode no $ENABLE_MODULE_LOAD &
...
valgrind -s --leak-check=full --show-leak-kinds=all  --verbose   --show-reachable=yes  --track-origins=yes --log-file="$SAVED/sys_tests/run_logs/_valgrind-6384.log"  src/redis-server  --bind 0.0.0.0 --port 6384 --dir ./data --dbfilename 6384.rdb --logfile "$SAVED/sys_tests/run_logs/_valgrind-redis-6384.log" --maxmemory 4G --protected-mode no $ENABLE_MODULE_LOAD &
echo $SAVED
cd $SAVED
sleep 10
...
../src/redis-cli -p 6382 monitor >$SAVED/sys_tests/run_logs/_monitor-6383.log &
../src/redis-cli -p 6384 monitor >$SAVED/sys_tests/run_logs/_monitor-6384.log &
pwd
