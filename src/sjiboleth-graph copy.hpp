...
class Graph_Leg;
typedef std::map<const char *, Graph_Leg *> TouchesMapType;
...
public:
    ...
    static Graph_Leg *Add(rxString key, double const weight, Graph_Leg *origin, GraphStack<Graph_Leg> *bsf_q, TouchesMapType *touches, int optimization, rxString contra_key)
    {
        if (touches)
        {
            TouchesMapType::iterator iter = touches->find(key);
            if(iter != touches->end())
                return NULL;
        }
        ...
        if (touches)
        {
            touches->insert(std::pair<const char *, Graph_Leg *>(key, leg));
            if (contra_key)
            {
                leg->Claim();
                touches->insert(std::pair<const char *, Graph_Leg *>(contra_key, leg));
            }
        }
        return leg;
    }
