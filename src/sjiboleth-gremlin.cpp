...
FaBlok *persistTriplet(SilNikParowy_Kontekst *stack, FaBlok *edge_set, const char *subject, const char *fwd_predicate, const char *bwd_predicate, const char *object, double weight, redisNodeInfo *data_config, GraphStack<const char> *fields_to_copy, void *originO);
...
extern const char *ValidateString(const char *s, size_t l)
{
    ...
    size_t pl = 0;
    while ((*p >= 0x20 && *p <= 0x7f) || (*p >= 0xa0 && ((short)*p) <= 0xff))
    {
        ++p;
        ++pl;
    }
...
static int redis_HSET_EDGE(SilNikParowy_Kontekst *stack, const char *key, const char *field, const char *value, const char *inverse_key, const char *host_reference, bool setMetaType)
{
    ...
    if (setMetaType)
    {
        auto metatype = rxStringFormat("~%s", value);
...
int executeMultiMatch(ParserToken *, SilNikParowy_Kontekst *stack, FaBlok *pl, MatchParameters *parms)
{
    ...
    char leader_temp_setname[512];
    char terminator_temp_setname[512];    
    char setname[512];

    snprintf(leader_temp_setname,sizeof(leader_temp_setname),"MULTIMATCH:::PATH:::%lld::FROM", ustime());
    auto *from = FaBlok::New(leader_temp_setname, KeysetDescriptor_TYPE_GREMLINSET);
    snprintf(terminator_temp_setname,sizeof(terminator_temp_setname),"MULTIMATCH:::PATH:::%lld::TO", ustime());
    auto *to = FaBlok::New(terminator_temp_setname, KeysetDescriptor_TYPE_GREMLINSET);
    snprintf(setname,sizeof(setname),"MULTIMATCH:::PATH:::%lld::OUT", ustime());
...
{
    // rxServerLog(rxLL_NOTICE, "#executeMatch# 0000  tid:%d stack:%p token:%p", (int)gettid(), stack, t);
    STACK_CHECK(1);
    ...
    else if (pl->IsParameterList())
    {
        ...
        char leader_temp_setname[MAX_ID_LENGTH];
        snprintf(leader_temp_setname, sizeof(leader_temp_setname), "%s:::PATH:::%lld::FROM", t->Token(), ustime());
        FaBlok *from = FaBlok::New(leader_temp_setname, KeysetDescriptor_TYPE_GREMLINSET);
        char terminator_temp_setname[MAX_ID_LENGTH];
        snprintf(terminator_temp_setname, sizeof(terminator_temp_setname), "%s:::PATH:::%lld::TO", t->Token(), ustime());
        ...
        {
            char leader[MAX_ID_LENGTH];
            rxCopyString(leader, sizeof(leader), leadersIterator.key, leadersIterator.key_len);
            if (rxStringCharCount(leader, ':') < 2)
            {
                ...
                {
                    ++tally;
                    char terminator[MAX_ID_LENGTH]; 
                    rxCopyString(terminator, sizeof(terminator), terminatorsIterator.key, terminatorsIterator.key_len);
        ...
        t->AggregateSizeSizes(tally, result->size);
    ...
    // rxServerLog(rxLL_NOTICE, "#executeMatch# 9999  tid:%d stack:%p token:%p", (int)gettid(), stack, t);
}
...
SJIBOLETH_HANDLER(GremlinDialect::executeNomatch)
{
    ...
    if (pl->IsParameterList())
    {
        ...
        t->AggregateSizeSizes(tally, nomatches->size);
        rxStringFree(setname);
        delete cpy;
    }
    else
...
SJIBOLETH_HANDLER(GremlinDialect::executeGremlinMatchInExclude)
{
    ...
    while (q.HasEntries())
    {
        ...
        if (members)
        {
            if (rxHarvestSetmembersInRax(metatype, members, filter))
            {
                raxIterator ri;
                raxStart(&ri, filter);
                raxSeek(&ri, ">#", NULL, 0);
                while (raxNext(&ri))
                {
                    if (ri.key[0] != '#')
                        break;
                    rxString m = rxStringNewLen((const char *)ri.key, ri.key_len);
                    q.Push(m);
                    rxStringFree(m);
                }
                raxSeek(&ri, ">~", NULL, 0);
    ...
    t->AggregateSizeSizes(0, raxSize(filter));
}
...
FaBlok *LoadKeySetFromMetaType(FaBlok *kd, rxString metaType)
{
    ...
    while (q.HasEntries())
    {
        ...
        while (auto member = (char *)rxGetSetMember(mob))
        {
            if (member[0] == '#' || member[0] == '~')
            {
                q.Push(strdup(member));
                continue;
            }
...
SJIBOLETH_HANDLER(GremlinDialect::executeAllVertices)
{
    ...
    t->AggregateSizeSizes(0, kd->size);
...
SJIBOLETH_HANDLER(GremlinDialect::executeGremlinParameters)
{
    ...
    char setname[128];
    snprintf(setname, sizeof(setname), "P_%lld", ustime());
...
SJIBOLETH_HANDLER(executeGremlinAs)
{
    ...
    stack->Memoize(sd->setname, base->Copy(NULL, KeysetDescriptor_TYPE_GREMLIN_VERTEX_SET + KeysetDescriptor_TYPE_GREMLIN_AS_SET));
    FaBlok::Delete(sd);
...
SJIBOLETH_HANDLER(executeGremlinUse)
{
    ...
    PushResult(sd, stack);
    t->AggregateSizeSizes(0, sd->size);
}
...
static SJIBOLETH_HANDLER(executeGremlinHas)
{
    ...
    */
    ...
    if (od)
    {
        snprintf(set_name, sizeof(set_name), "%s:%s==%s", od->AsSds(), ad->AsSds(), vd->AsSds());
    }
    else
    {
        snprintf(set_name, sizeof(set_name), "%s==%s", ad->AsSds(), vd->AsSds());
    }

    FaBlok *kd = NULL;
    char meta_name[2048];
    char *pMeta_Name = meta_name;
    snprintf(meta_name, sizeof(meta_name), "#%s", vd->AsSds());
    void *meta_set = rxFindSetKey(0, pMeta_Name);
    if (meta_set)
    {
        FaBlok *base_set = FaBlok::New(meta_set, KeysetDescriptor_TYPE_GREMLIN_VERTEX_SET);
        rxHarvestSetmembersInRax(meta_name, meta_set, base_set->keyset);

        kd = od->CopyAnd(base_set);
        FaBlok::Delete(base_set);
    }
    else
    {
        ...
        kd = od->Copy(set_name, KeysetDescriptor_TYPE_GREMLINSET, FilterTypes, params);
    }
    kd->AsTemp();

    t->AggregateSizeSizes(od->size, kd->size);
    PushResult(kd, stack);
}
...
int traverseAdjacent(FaBlok *leaders, FaBlok *kd, MatchParameters *parms);
...
int executeGremlinTraverse(SilNikParowy_Kontekst *stack, MatchParameters *parms)
{
    ...
    */
    ...
    traverseAdjacent(od, kd, parms);
    PushResult(kd, stack);

    // while(listLength(patterns) > 0 ){
    //     auto ln = listFirst(patterns);
    //     listGetFreeMethod(patterns)
    //     rxMemFree(ln->value);
    //     listDelNode(patterns, ln);
    // }
    // listSetFreeMethod(patterns, rxMemFree);
    listRelease(patterns);
    rxStringFree(set_name);
    MatchParameters::Delete(stack);
    return C_OK;
...
// Do a breadth first for any vertex or edge of type <out>
/*
    rxquery: g:v(...).out(edge_type)

    Interpretation: Find the objects of all triplets for which v(...) is the subject.


    rxquery: g:v(...).in(edge_type)

    Interpretation: Find the subjects of all triplets for which v(...) is the object.
    ...
    * S vertex
    ...
    rxquery g:v(F).out('has boss')
                                    result: S

    rxquery g:v(F).in('has boss')
                                    result: empty

    rxquery g:v(S).out('has boss')
                                    result: empty

    rxquery g:v(S).in('has boss')
                                    result: F

    rxquery g:v(S).out('boss of')
                                    result: F

    rxquery g:v(S).in('boss of')
                                    result: empty


    rxquery g:v(F).out('boss of')
                                    result: empty

    rxquery g:v(F).in('boss of')
                                    result: F
...
FaBlok *createVertex(SilNikParowy_Kontekst *stack, FaBlok *v, const char *key, const char *vertex_type)
{
    ...
    redis_HSET(stack, key, Graph_vertex_or_edge_type, vertex_type, data_config->host_reference);
    o = rxFindKey(0, key);
    if (v)
        v->AddKey(key, o);
    rxString metatype = rxStringFormat("#%s", vertex_type);
    redis_SADD(stack, metatype, key, data_config->host_reference);
    rxStringFree(metatype);
    return v;
}
...
FaBlok *createVertex(SilNikParowy_Kontekst *stack, FaBlok *pl)
{
    ...
    FaBlok *out = FaBlok::New(i->setname, KeysetDescriptor_TYPE_GREMLINSET);

    return createVertex(stack, out, i->setname, t->setname);
}
...
public:
    ...
    static renamert *New(const char *edge, size_t edge_sz, renamert *scope)
    {
        ...
        char edge_type[256];
        char buffer[MAX_ID_LENGTH], *b = buffer;
...
void rename_keys(const char *old_key, const char *new_key, SilNikParowy_Kontekst *stack)
{
    ...
    while (rename_queue.HasEntries())
    {
        ...
        if (action->old_link)
        {
            rxSetMembers *mob = rxHarvestSetmembersForKey(0, action->old_link, &mob);
            while (auto member = (char *)rxGetSetMember(mob))
            {
                ...
                if (colons[1])
                {
                    ...
                    else
                    {
                        ...
                        rxSetMembers *mob = rxHarvestSetmembersForKey(0, buffer, &mob);
                        while (auto member = (char *)rxGetSetMember(mob))
                        {
...
void CopyFieldToEdge(const char *k, GraphStack<const char> *fields_to_copy, void *originO, SilNikParowy_Kontekst *stack, redisNodeInfo *data_config)
{
    if (!fields_to_copy)
        return;
    fields_to_copy->StartHead();
    const char *f;
    while ((f = fields_to_copy->Next()))
    {
        if (originO)
        {
            auto value = rxGetHashField(originO, f);
            if (value)
            {
                redis_HSET(stack, k, f, value, data_config->host_reference);
            }
        }
    }
}
/*
             ...
             .-------------.             .-------------.             .-------------.
*/
void AddMeta(const char *subject, const char *fwd_predicate, const char *bwd_predicate, const char *object, SilNikParowy_Kontekst *stack, redisNodeInfo *data_config)
{
    // return; // TODO: Fix hash error
    void *s = rxFindKey(0, subject);
    auto st = rxGetHashField(s, "type");
    if (!st)
        return;
    void *o = rxFindKey(0, object);
    auto ot = rxGetHashField(o, "type");
    if (!ot)
        return;
    char mf[4096];
    snprintf(mf, sizeof(mf), "META ~%s:#%s:#%s", fwd_predicate, st, ot);
    char mb[4096];
    snprintf(mb, sizeof(mb), "META ~%s:#%s:#%s", bwd_predicate ? bwd_predicate : fwd_predicate, ot, st);
    char *mi[] = {mf, mb, mf};
    for (size_t n = 0; n < 2; ++n)
    {
        // rxServerLog(rxLL_NOTICE, "#AddMeta# %ld %s %s", n, mi[n], mi[n + 1]);
        void *m = rxFindKey(0, mi[n]);
        if (!m)
        {
            redis_HSET_EDGE(stack, mi[n], "type", "meta", mi[n + 1], data_config->host_reference, true);
...
FaBlok *persistTriplet(SilNikParowy_Kontekst *stack, FaBlok *edge_set, const char *subject, const char *fwd_predicate, const char *bwd_predicate, const char *object, double weight, redisNodeInfo *data_config, GraphStack<const char> *fields_to_copy, void *originO)
{
    ...
    AddMeta(subject, fwd_predicate, bwd_predicate, object, stack, data_config);
    ...
    const char *bridges[2] = {fwd_predicate, bwd_predicate};
    for (int j = 0; j < 2; ++j)
    {
        redis_HSET_EDGE(stack, bridge_keys[j], Graph_vertex_or_edge_type, bridges[j], backLink, data_config->host_reference, true);
        if (fields_to_copy)
            CopyFieldToEdge(bridge_keys[j], fields_to_copy, originO, stack, data_config);
        if (strlen(backLink))
        {
            redis_HSET_EDGE(stack, bridge_keys[j], "edge", backLink, edge_name, data_config->host_reference, false);
            if (fields_to_copy)
                CopyFieldToEdge(bridge_keys[j], fields_to_copy, originO, stack, data_config);
        }
    }

    rxString keys[] = {fwd_predicate, subject, edge_link, object, bwd_predicate, object, inv_edge_link, subject};
    ...
    while ((raxNext(&subject_iterator)))
    {
        ...
        while ((raxNext(&object_iterator)))
        {
            ...
            if (strlen(predicates[1]) == 0 || strcmp(predicates[1], predicates[0]) == 0)
                edge_set = persistTriplet(stack, edge_set, subject_key, predicates[0], strlen(predicates[1]) ? predicates[1] : predicates[0], object_key, 1.0, data_config, NULL, NULL);
            else
            {
                // two predicates are considered indepents facts
                edge_set = persistTriplet(stack, edge_set, subject_key, predicates[0], predicates[1], object_key, 1.0, data_config, NULL, NULL);
                edge_set = persistTriplet(stack, edge_set, object_key, predicates[1], predicates[0], subject_key, 1.0, data_config, NULL, NULL);
            }
...
SJIBOLETH_HANDLER(executeGremlinLinkVertex)
{
    ...
    else
    {
        if (e == NULL)
        {
            FaBlok *output_set = v->CopyKeys(v->AsSds(), KeysetDescriptor_TYPE_GREMLINSET, (void *)&is_to);
            PushResult(output_set, stack);
            return C_OK;
        }
        if (v->size == 0)
...
void executeGremlinAddEdgeUsingProperty(SilNikParowy_Kontekst *stack, FaBlok *et, size_t no_vertex_parms, redisNodeInfo *data_config)
{
    ...
    const char *property = input_set->setname + 1;
    ...
    while (raxNext(&SubjectIterator))
    {
        ...
        if (objectKey == NULL)
        {
            objectKey = rxGetHashField(subject, property);
        }
...
void executeGremlinAddEdgeUsingSubjectEdgeNamesObject(SilNikParowy_Kontekst *stack, FaBlok *et, size_t no_vertex_parms, redisNodeInfo *data_config, ParserToken *t)
{
    // addE(dge)(<subject>, <type>, <object>)
    FaBlok *w = et->parameter_list->Last(); // Peek if last parameter is a float or a copy list
    GraphStack<const char> fields_to_copy;

    if (w->IsParameterList())
    {

        FaBlok *field_to_copy;
        while ((field_to_copy = w->parameter_list->Dequeue()))
        {
            fields_to_copy.Add(strdup(field_to_copy->setname));
        }
        FaBlok::Delete(w);
        no_vertex_parms--;
        w = et->parameter_list->Last();
    }
    else
        et->parameter_list->Enqueue(w);
    ...
    FaBlok *subject_name = et->parameter_list->Dequeue(); // iri of subject
    FaBlok *pred = et->parameter_list->Dequeue();         // predicate(type)
    FaBlok *inv_pred;
    if (no_vertex_parms >= 4)
        ...
        inv_pred = pred;
    FaBlok *object_name = /*stack->Pop(); //*/ et->parameter_list->Dequeue(); // iri of object

    if (*subject_name->setname == '@' || *object_name->setname == '@')
    {
        FaBlok *data = stack->Pop();
        FaBlok *out = FaBlok::New("Dynamic Triplets", KeysetDescriptor_TYPE_GREMLIN_EDGE_SET);
        raxIterator dynamicTripletIterator;
        raxStart(&dynamicTripletIterator, data->keyset);
        raxSeek(&dynamicTripletIterator, "^", NULL, 0);
        while (raxNext(&dynamicTripletIterator))
        {
            rxString key = rxStringNewLen((const char *)dynamicTripletIterator.key, dynamicTripletIterator.key_len);
            auto subjectKey = rxGetHashField(dynamicTripletIterator.data, subject_name->setname + 1);
            void *subjectO = rxFindHashKey(0, subjectKey ? subjectKey : key);
            if (!subjectO)
            {
                createVertex(stack, out, subjectKey, subject_name->setname + 1);
            }
            auto objectKey = rxGetHashField(dynamicTripletIterator.data, object_name->setname + 1);
            void *objectO = rxFindHashKey(0, objectKey ? objectKey : key);
            if (!objectO)
            {
                createVertex(stack, out, objectKey ? objectKey : key, object_name->setname + 1);
            }
            rxString fwd = NULL;
            rxString bwd = NULL;
            if (*pred->setname == '@')
                fwd = rxGetHashField(dynamicTripletIterator.data, pred->setname + 1);
            else
                fwd = rxStringNew(pred->setname);
            if (!fwd)
            {
                fwd = rxGetHashField(dynamicTripletIterator.data, pred->setname + 1);
                fwd = "?";
            ...
            {
                if (*inv_pred->setname == '@')
                    bwd = rxGetHashField(dynamicTripletIterator.data, inv_pred->setname + 1);
                else
                    bwd = rxStringNew(inv_pred->setname);
            }

            persistTriplet(stack, out, subjectKey ? subjectKey : key, fwd, bwd, objectKey ? objectKey : key, weight, data_config, &fields_to_copy, dynamicTripletIterator.data);
            ...
            //     else
            //         bwd = rxStringNew(inv_pred->setname);
            //     persistTriplet(stack, out, objectKey ? objectKey : key, bwd, fwd, subjectKey ? subjectKey : key, weight, data_config, &fields_to_copy, dynamicTripletIterator.data);
            // }
        }
        raxStop(&dynamicTripletIterator);
        t->AggregateSizeSizes(data->size, out->size);
        FaBlok::Delete(data);
        PushResult(out, stack);
    }
    else
    {
        FaBlok *e = persistTriplet(stack, NULL, subject_name->setname, pred->setname, pred->setname, object_name->setname, weight, data_config, NULL, NULL);
        if (strcmp(pred->setname, inv_pred->setname) != 0)
            e = persistTriplet(stack, e, object_name->setname, inv_pred->setname, inv_pred->setname, subject_name->setname, weight, data_config, NULL, NULL);
        PushResult(e, stack);
    }
    FaBlok::Delete(et);
    // delete copies
    // while ((fields_to_copy.HasEntries()))
    // {
    //     auto f = fields_to_copy.Pop();
    //     rxMemFree((void *)f);
    // }
}
    ...
    FaBlok *bwd_predicate)
{
    ...
    char temp_key[2048];
    snprintf(temp_key, sizeof(temp_key), "%s:%s::%lld", fwd_predicate->setname, bwd_predicate ? bwd_predicate->setname : "", ustime());
...
SJIBOLETH_HANDLER(executeGremlinAddEdge)
{
    ...
    {
        is_link_from_property = rxStringMatchLen(et->parameter_list->Peek()->setname, 1, "@", 1, 1);
    }
    ...
    {
        executeGremlinAddEdgeUsingSubjectEdgeNamesObject(stack, et, no_vertex_parms, data_config, t);
    }
...
SJIBOLETH_HANDLER(executeGremlinAddTriplet)
{
    ...
    // Get parameters g:addTriplet([@]<subject>, [@]<predicate>, [@]<object> [,( attribute0, ..., attribute_n ) ])
    // Get parameters g:addTriplet([@]<subject>, [@]<predicate>, [@]<object>, <weight> [,( attribute0, ..., attribute_n ) ])
    // Get parameters g:addTriplet([@]<subject>, [@]<predicate>, [@]<inverse predicate>, [@]<object> [,( attribute0, ..., attribute_n ) ])
    // Get parameters g:addTriplet([@]<subject>, [@]<predicate>, [@]<inverse predicate>, [@]<object>, <weight> [,( attribute0, ..., attribute_n ) ])
    redisNodeInfo *data_config = rxDataNode();
...
SJIBOLETH_HANDLER(executeGremlinCloveTriplet)
{
    // getRxSuite()->debugMessages = 16924;
    // rxServerLog(rxLL_NOTICE, "#executeGremlinCloveTriplet# %d, stack: %p", (int)gettid(), stack);
    // stack->DumpStack();
    // Get parameters g:cloveTriplet((<subject>), (<predicate>), (<object>), (<fields_to_move>))
    // <subject> ::= ( @<subject field>, @<subject type field>) | @key
    // <predicate> ::= ( <fwdLink> {, <bwdLink> {, @<weight field>})
    // <object> ::= ( @<object field>, @<object type field>) | @key
    // <fields_to_move> ::= ( <field>, ...)
    //
    // Example:
    //
    //         G V(town).cloveTriplet( (@admin_name, admin_region), (admin_for, town_of), @key, (country, iso2, iso3) )
    //         G V(admin_region).cloveTriplet( (@country, land), (country_for, admin_in), @key, (iso2, iso3) )
    // moves <field>... into a new vertext.
    // creates a triplet between V and new V
    //
    // Warning:
    // * When properties are moved than both keys are 're-triggered'!
    //   Add a hasLabel step function to the IF clause to prevent re-cloving
    //

    double weight = 1.0;
    redisNodeInfo *data_config = rxDataNode();
    FaBlok *cloveParameters = stack->Pop();
    if (!cloveParameters->parameter_list)
    {
        rxServerLog(rxLL_NOTICE, "#executeGremlinCloveTriplet# stack malformed %d, stack: %p", (int)gettid(), stack);
        ERROR("executeGremlinCloveTriplet stack malformed");
    }
    FaBlok *subject_name = NULL;
    FaBlok *subject_type = NULL;
    FaBlok *fwd_link_name = NULL;
    FaBlok *bwd_link_name = NULL;
    FaBlok *object_name = NULL;
    FaBlok *object_type = NULL;

    subject_name = cloveParameters->parameter_list->Dequeue();
    if (!subject_name)
    {
        rxServerLog(rxLL_NOTICE, "#executeGremlinCloveTriplet# SUBJECT_NAME # stack malformed %d, stack: %p", (int)gettid(), stack);
        ERROR("executeGremlinCloveTriplet stack malformed# SUBJECT_NAME #");
    }
    if (!rxStringMatch("@key", subject_name->setname, 1))
        subject_type = cloveParameters->parameter_list->Dequeue();
    object_name = cloveParameters->parameter_list->Dequeue();
    if (!object_name)
    {
        rxServerLog(rxLL_NOTICE, "#executeGremlinCloveTriplet# OBJECT_NAME # stack malformed %d, stack: %p", (int)gettid(), stack);
        ERROR("executeGremlinCloveTriplet stack malformed# OBJECT_NAME #");
    }
    if (!rxStringMatch("@key", object_name->setname, 1))
        object_type = cloveParameters->parameter_list->Dequeue();
    FaBlok *predicateRule = cloveParameters->parameter_list->Dequeue();
    if (!predicateRule)
    {
        rxServerLog(rxLL_NOTICE, "#executeGremlinCloveTriplet# PREDICATE # stack malformed %d, stack: %p", (int)gettid(), stack);
        ERROR("executeGremlinCloveTriplet stack malformed# PREDICATE #");
    }
    if (predicateRule && predicateRule->IsParameterList())
    {
        fwd_link_name = predicateRule->parameter_list->Dequeue();
        bwd_link_name = predicateRule->parameter_list->Dequeue();
    }
    else
        fwd_link_name = predicateRule;

    GraphStack<const char> movers;
    FaBlok *aMover;
    while ((aMover = cloveParameters->parameter_list->Dequeue()))
    {
        movers.Add(aMover->setname);
    }
    if (subject_type)
        movers.Add(subject_name->setname + 1);
    if (object_type)
        movers.Add(object_name->setname + 1);

    FaBlok *data = stack->Pop();
    FaBlok *out = FaBlok::New("Dynamic Triplets", KeysetDescriptor_TYPE_GREMLIN_EDGE_SET);
    if (data)
    {
        raxIterator cloveObjectIterator;
        raxStart(&cloveObjectIterator, data->keyset);
        raxSeek(&cloveObjectIterator, "^", NULL, 0);
        while (raxNext(&cloveObjectIterator) && out->size < 10000)
        {
            rxString key = rxStringNewLen((const char *)cloveObjectIterator.key, cloveObjectIterator.key_len);

            auto subjectKey = rxGetHashField(cloveObjectIterator.data, subject_name->setname + 1);
            auto objectKey = rxGetHashField(cloveObjectIterator.data, object_name->setname + 1);

            int new_objects = 0;
            rxString subjectRef = NULL;
            rxString objectRef = NULL;
            if (subject_type)
            {
                auto subjectType = rxGetHashField(cloveObjectIterator.data, subject_type->setname + 1);
                subjectRef = getSha1("ss", subjectKey, subject_type->setname);
                void *subjectO = rxFindHashKey(0, subjectKey ? subjectKey : key);
                if (!subjectO)
                {
                    createVertex(stack, out, subjectRef, subjectType ? subjectType : subject_type->setname);
                    ++new_objects;
                }
            }
            else
            ...
            if (object_type)
            {
                auto objectType = rxGetHashField(cloveObjectIterator.data, object_type->setname + 1);
                objectRef = getSha1("ss", objectKey, object_type->setname);
                void *objectO = rxFindHashKey(0, objectKey ? objectKey : key);
                if (!objectO)
                {
                    createVertex(stack, out, objectRef ? objectRef : key, objectType ? objectType : object_name->setname);
                    ++new_objects;
                }
            }
            else
            ...
            else
            {
                ...
                while ((f = movers.Next()))
                {
                    if (new_objects)
                    {
                        auto value = rxGetHashField(cloveObjectIterator.data, f);

                        if (value)
                        {
                            redis_HSET(stack, subjectRef ? subjectRef : objectRef, f, value, data_config->host_reference);
                            redis_HDEL(stack, key, f, data_config->host_reference);
                        }
                    }
                    else
                        redis_HDEL(stack, key, f, data_config->host_reference);
            ...
            persistTriplet(stack, out, subjectRef ? subjectRef : key, fwd_link_name ? fwd_link_name->setname : "?", bwd_link_name ? bwd_link_name->setname : (fwd_link_name ? fwd_link_name->setname : "!"), objectRef ? objectRef : key, weight, data_config, NULL, NULL);
        }
        raxStop(&cloveObjectIterator);
    }
    FaBlok::Delete(cloveParameters);
    t->AggregateSizeSizes(data ? data->size : 0, out->size);

    PushResult(out, stack);
}
END_SJIBOLETH_HANDLER(executeGremlinCloveTriplet)
...
SJIBOLETH_HANDLER(executeWhereExpression)
{
    ...
    t->AggregateSizeSizes(input_set->size, output_set->size);
    FaBlok::Delete(input_set);
    PushResult(output_set, stack);
}
...
SJIBOLETH_HANDLER(executePushDupStack)
{
    ...
    {
        FaBlok *input_set = stack->Pop();
        PushResult(input_set->Copy(NULL, input_set->value_type), stack);
        PushResult(input_set, stack);
        stack->DumpStack();
    }
...
bool GremlinDialect::RegisterDefaultSyntax()
{
    ...
    this->RegisterSyntax("cloveTriplet", 500, 2, 1, Q_WRITE, &executeGremlinCloveTriplet);
...
int AddMemberToKeysetForMatch(int db, unsigned char *vstr, size_t vlen, FaBlok *kd, Graph_Leg *terminal, unsigned char * /*key*/, MatchParameters *parms)
{
    ...
    if (members != NULL)
    {
        ...
        while (auto member = (char *)rxGetSetMember(mob))
        {
            rxServerLog(rxLL_DEBUG, "AddMemberToKeysetForMatch for %s : %ld: %s", edge_key, mob->member_index, member);
            int msegments = 0;
...
bool IsKeyApplicable(const char *key, MatchParameters *parms)
{
    ...
    if (!parms->excludes && !parms->includes)
        return true;
    return false;
}
...
int matchEdges(int db, Graph_Leg *leg, FaBlok *kd, GraphStack<Graph_Leg> *bsf_q, rax *touches, rax *terminators, MatchParameters *parms)
{
    ...
    snprintf(key, sizeof(key), "^%s", leg->key);
    int numkeys = 0;

    rxSetMembers *mob = rxHarvestSetmembersForKey(db, key, &mob);
    if (mob)
        while (auto elesds = (char *)rxGetSetMember(mob))
        {
            ...
            char link[2048];
            strncpy(link, (parts[1]) + 1, sizeof(link));
            // rxString link = rxStringNew(parts[1] + 1);
            int link_segments = 0;
            rxString *link_parts = rxStringSplitLen(link, strlen(link), ":", 1, &link_segments);
            ...
            }
            {
                ...
                if (rxGetObjectType(next_obj) == rxOBJ_HASH)
                {
                    ...
                    else
                    {
                        ...
                        {
                            double value = atof(optimiseFieldValue) * 2; // Will be halfed later
                            if (value != 0)
                                weight = value;
                        }
                    }
                ...
                Graph_Leg *next = NULL;
                if (IsKeyApplicable(link, parms))
                {
                    next = Graph_Leg::Add(link, weight * parms->optimization, leg, bsf_q, touches, 1, (half && edge) ? ((strcmp(half, link) == 0) ? edge : half) : NULL);
                }
                if (next)
                {
                    next->obj = next_obj;
...
int breadthFirstSearch(FaBlok *leaders, FaBlok *kd, rax *terminators, MatchParameters *parms)
{
    ...
    if (leaders && raxSize(leaders->keyset) > 0)
    {
        ...
        {
            char key[MAX_ID_LENGTH];
            rxCopyString(key, sizeof(key), leadersIterator.key, leadersIterator.key_len);
            if (IsKeyApplicable(key, parms))
            {
...
/*
    For each input vertex:
        Fetch the edge members matching the type
            Fetch the subject or object vertex as requested.
...
int traverseAdjacent(FaBlok *leaders, FaBlok *kd, MatchParameters *parms)
{
    ...
    while (raxNext(&leadersIterator))
    {
        ...
        while (auto elesds = (char *)rxGetSetMember(mob))
        {
            rxServerLog(rxLL_DEBUG, "traverseAdjacent for %s : %ld: %s", key, mob->member_index, elesds);
            char *colon[8];
            breakupPointer((char *)elesds, '|', colon, 8);
            ...
            if (doesMatchOneOfThePatterns == 0 && parms->patterns != NULL)
            {
                ...
                while ((ln = listNext(li)) && doesMatchOneOfThePatterns == 0)
                {
                    ...
                    doesMatchOneOfThePatterns += (rxStringMatchLen(pattern, strlen(pattern), elesds, colon[1] - elesds, 1)) ? 1 : 0;
                }
                listReleaseIterator(li);
            }
            if (doesMatchOneOfThePatterns != 0)
            {
                rxString pointer = rxStringNewLen(colon[1] + 1, colon[2] - colon[1] - 1);
                void *edge = rxFindSetKey(0, pointer);
                if (edge == NULL)
                {
                ...
                while (auto elesds = (char *)rxGetSetMember(pointers))
                {
                    rxServerLog(rxLL_DEBUG, "traverseAdjacent target for %s : %ld: %s", key, mob->member_index, elesds);
                    char *bars[8];
                    breakupPointer(elesds, '|', bars, 8);
                    switch (parms->traverse_direction)
                    {
                    case GRAPH_TRAVERSE_IN:
                    {
                        char *IN = strstr(elesds, "ES");
                        if (IN == NULL)
                            continue;
                        break;
                    }
                    case GRAPH_TRAVERSE_OUT:
                    {
                        char *OUT = strstr(elesds, "EO");
                        if (OUT == NULL)
                            continue;
                        break;
                    }
